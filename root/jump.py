...
# ================= DEFAULTS =================
DEFAULT_URL     = "https://partners.streakads.com/click?aid=3&oid=413"
DEFAULT_VISITS  = 1000
DEFAULT_GEOS    = "IN"
LOG_FILE        = "jump_visits.json"
TEXT_LOG_FILE   = "jump_visits.txt"

# The playable ad domain (Chicken Road game). We stay on the ad while jumping,
# and the click-out lands us on the real offer (join4ra.com etc.).
AD_DOMAIN = "4raqqpwa"
    ...
    try:
      async with async_playwright() as p:
        ...
        try:
            ...
            jump_btn = page.locator('button:has-text("JUMP")')
            install_btn = page.locator('button:has-text("Install")')

            # Wait for the JUMP button to appear (game loaded)
            await jump_btn.first.wait_for(state="visible", timeout=35000)

            # 2) Keep tapping JUMP until the Install card appears
            for _ in range(40):  # safety cap so it can't loop forever
                if await install_btn.count() > 0 and await install_btn.first.is_visible():
                    break

                if await tap_or_click(jump_btn):
                    jumps += 1
                    log(f"   🐔 JUMP #{jumps}")
            ...
            if await install_btn.count() > 0 and await install_btn.first.is_visible():
                ...
                log(f"   ✅ Visit #{run_number} done ({jumps} jumps)")
            else:
