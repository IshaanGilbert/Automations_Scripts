...
# GoLogin's SDK verifies HTTPS during its proxy pre-check (GET https://geo.myip.link
# through the SOCKS proxy). On a minimal server the CA bundle is missing -> "unable to get
# local issuer certificate" -> "Proxy check failed" -> Orbita won't start. Point
# requests/urllib3 at certifi's CA bundle so that check passes.
try:
    import certifi
...
# GoLogin SDK — drives a REAL anti-detect browser (Orbita) per account, instead of a raw
# Chromium + our own fingerprint.py spoofing. Install with:  ./venv/bin/pip install gologin
try:
    from gologin import GoLogin
except Exception:
    GoLogin = None   # checked at run time -> clear message instead of an import crash
...
# Password to LOG IN / create-account when verifying a purchase via my_orders (see
# verify_via_login). Empty => skip login-verify.
...
# ================= GOLOGIN =================
# GoLogin runs an anti-detect browser (Orbita). Set GOLOGIN_TOKEN in config.json (API
# token from gologin.com -> Settings -> API). If GOLOGIN_PROFILE_ID is set, that ONE
# profile is reused; otherwise a fresh profile is created per account (carrying the India
# proxy) and deleted afterwards so profiles don't pile up.
GOLOGIN_TOKEN = (str(config.get("GOLOGIN_TOKEN") or "")).strip()
GOLOGIN_PROFILE_ID = (str(config.get("GOLOGIN_PROFILE_ID") or "")).strip()
# Run Orbita headless? On a server it runs inside Xvfb, so headless can stay False.
GOLOGIN_HEADLESS = bool(config.get("GOLOGIN_HEADLESS", False))
...
def _solve_hcaptcha_paid(page, api_key, timeout, sitekey):
    """OLD paid-solver path — only used if CAPTCHA_CLICK_ONLY is off."""
    if not sitekey:
    ...
    if not key:
        ...
        return False
    pageurl = page.url
    # Match the solver's IP + User-Agent to OUR browser (Gemini points 2 & 3) — the single
    # biggest reason a "solved" token gets rejected by the site.
    try:
        ua = page.evaluate("() => navigator.userAgent")
    except Exception:
        ua = None
    proxy = _solver_proxy()
    print(f"   🧩 hCaptcha detected (sitekey {sitekey[:10]}…) — solving via {provider}"
          f"{' (proxy+UA matched)' if proxy else ' (UA matched)' if ua else ''}", flush=True)
...
def solve_hcaptcha(page, api_key=None, timeout=CAPTCHA_TIMEOUT):
    """No paid solver (default): CLICK the hCaptcha "I am human" checkbox. Returns True if it
    passes, False if it throws an image challenge or times out, 'none' if no captcha."""
    ...
    for fr in list(page.frames):
        u = (fr.url or "")
        if "hcaptcha.com" in u and "frame=challenge" not in u:
            for sel in ['#checkbox', 'div[role="checkbox"]', '#anchor', '.check']:
                try:
                    el = fr.locator(sel).first
                    if el.count() and el.is_visible():
                        el.click(timeout=5000)
                        print("   🖱️ checkbox clicked", flush=True); break
                except Exception:
                    pass
            break
    ...
    while time.time() < deadline:
        if _token_present():
            print("   ✅ hCaptcha passed via checkbox", flush=True); return True
        for fr in list(page.frames):
            if "frame=challenge" in (fr.url or ""):
                print("   ❌ hCaptcha threw an image challenge — no solver, failing fast", flush=True); return False
        time.sleep(1)
...
def verify_via_login(context, email, name, password=None):
    ...
    try:
        vp = context.new_page()
        try: vp.set_default_timeout(12000)
        except Exception: pass
...
def run_task(task, proxy_port, proxy_ip):
    ...
    gl = None; created_profile = None       # GoLogin handle + per-account profile to clean up
    ...
    try:
        t0 = time.time()
        if GoLogin is None:
            raise Exception("gologin module not installed — run: ./venv/bin/pip install gologin")
        if not GOLOGIN_TOKEN:
            raise Exception("GOLOGIN_TOKEN not set in config.json")
        pw = sync_playwright().start()

        # ---- GoLogin anti-detect browser (Orbita) ----
        # GoLogin provides the FULL fingerprint (UA / canvas / WebGL / fonts / WebRTC) — no
        # fingerprint.py needed. Use a fixed profile if GOLOGIN_PROFILE_ID is set, else
        # create a fresh profile per account carrying the India proxy, and delete it at the
        # end so profiles don't pile up.
        gl = GoLogin({
            "token": GOLOGIN_TOKEN,
            "tmpdir": os.path.join(os.path.expanduser("~"), ".gologin_tmp"),  # writable temp
            # Orbita (Chromium) refuses to run as root without --no-sandbox; the bots run
            # as root, so pass server-friendly flags through to Orbita.
            "extra_params": ["--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu"],
        })
        _gl_os = "win" if sys.platform == "win32" else "mac" if sys.platform == "darwin" else "lin"
        if GOLOGIN_PROFILE_ID:
            gl.setProfileId(GOLOGIN_PROFILE_ID)
            print(f"   🧬 GoLogin: using fixed profile {GOLOGIN_PROFILE_ID}", flush=True)
        else:
            # Random fingerprint — EXACTLY the proven working script's params (os win,
            # timezone Asia/Kolkata, language en-US + languages). GoLogin fills a valid
            # UA/canvas/WebGL; language set here in the create call.
            profile = gl.createProfileRandomFingerprint({
                "name": f"mz-{re.sub(r'[^A-Za-z0-9]', '', email)[:24]}",
                "os": "win",
                "timezone": "Asia/Kolkata",
                "language": "en-US",
                "languages": ["en-US", "en"],
                # Device language को भी force करें
                "device": {
                    "language": "en-US",
            ...
            created_profile = profile["id"] if isinstance(profile, dict) else profile
            print(f"   🧬 GoLogin: created profile {created_profile} (Language: en-US)", flush=True)
            if PROXY_HOST and PROXY_USERNAME:
                try:
                    # Proxy को test करें
                    print(f"   🔧 Configuring proxy: {PROXY_HOST}:{PROXY_PORT}", flush=True)
                    
                    gl.changeProfileProxy(created_profile, {
                        "mode": "socks5",
                        "host": str(PROXY_HOST),
                        "port": int(PROXY_PORT),
                    ...
                    print(f"   ✅ Proxy configured successfully", flush=True)
                except Exception as proxy_err:
                    print(f"   ⚠️ Proxy setup failed: {proxy_err}", flush=True)
                    # Proxy के बिना continue करें
            gl.setProfileId(created_profile)
            print(f"   🧬 GoLogin: created random-fingerprint profile {created_profile}", flush=True)

        # Start Orbita + connect Playwright over CDP. Retry a few times (Orbita download
        # / start can be slow the first time, or hit a transient).
        browser = None
        for _bl_try in range(3):
            try:
                debugger_address = gl.start()                 # "127.0.0.1:<port>"
                browser = pw.chromium.connect_over_cdp(f"http://{debugger_address}")
                break
            except Exception as _bl_e:
                print(f'   ⚠️ GoLogin start/connect failed (try {_bl_try+1}/3): {str(_bl_e)[:120]}', flush=True)
                try: gl.stop()
                except Exception: pass
                time.sleep(10)
                if _bl_try == 2:
                    raise
        # GoLogin opens with a default context — reuse it (don't make a new context, that
        # would lose the profile's fingerprint).
    ...
    """)
        ...
        # JavaScript में भी language set करें
        page.evaluate("""() => {
        ...
        # On 'not eligible' pick a RANDOM mutated email and re-enter it on the SAME page
        ...
        for _stripe_poll in range(45):
            ...
            # Reload only embedded Stripe (old flow); never reload hosted checkout.
            if _stripe_poll == 22 and not _stripe_reloaded and 'checkout.stripe.com' not in page.url:
                print("   ⚠️ Stripe iframe slow (45s) — reloading page to recover...", flush=True)
                try:
                    page.reload(timeout=20000, wait_until="domcontentloaded")
                    time.sleep(5)
                except Exception: pass
                _stripe_reloaded = True
        ...
        for corp_try in range(1, 4):                      # 3 attempts (2 reloads)
            ...
            print(f"   ⏳ Waiting for corporate form (try {corp_try}/3, up to 2 min)...")
        ...
        with db.otp_lock(emp_id):
            ...
            for otp_attempt in range(1, MAX_OTP_ATTEMPTS+1):
                ...
                invalid=False
                # Bounded wait on THIS page for the thank-you (reload if it lands blank —
                # the post-payment redirect is slow/flaky). Kept ~80s; the login→my_orders
                # check below is the definitive proof, so we don't hang here.
                blank=0
                for i in range(40):
                    try:
            ...
            # DEFINITIVE verify — thank-you page often never loads after payment, so LOG IN
            # with the same email in a fresh tab and read the Transaction ID from my_orders
            # (account order-history = real proof). Needs VERIFY_LOGIN_PASSWORD in config.
            if not confirmed:
        ...
        for closer in (
            ...
            lambda: gl.stop() if gl else None,                      # stop Orbita
            # delete the per-account profile we created (skip a fixed GOLOGIN_PROFILE_ID)
            lambda: gl.delete(created_profile) if (gl and created_profile) else None,
        ):
            try: closer()
            except: pass
...
def main():
    ...
    while True:
        # GLOBAL PAUSE — operator kill-switch (see buyer.py). Wait & re-poll while paused;
        # never exit, so the bot resumes the moment pause is lifted.
        if db.is_paused():
