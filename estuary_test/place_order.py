...
# Card fill auto; bank OTP manual.
# Updated: robust Credit/Debit Card pick + fail-stop if card not selected.
# """
# import os
# import re
# import sys
...
# GEONODE_PASS = "CHANGE_ME_SECRET"
# GEONODE_HOST = "proxy.geonode.io"
# GEONODE_PORT = "12000"          # Sticky port (11000 = Rotating)
# GEONODE_COUNTRY = "in"
...
#     "[class*='payment']:has-text('Credit/Debit Card')",
...
# def pick_card(page):
#     """Credit/Debit Card chuno -- robust selectors + JS fallback + radio force."""
#     f = sr_frame_any(page)
#     if not f:
#         return "checkout ka frame nahi mila"
...
#                 if not l.count():
#                     continue
#                 for i in range(min(l.count(), 6)):
#                     el = l.nth(i)
#                     try:
#                         if not el.is_visible():
#                             continue
#                         box = el.bounding_box()
#                         if box and box["height"] > 5 and box["width"] > 20:
#                             loc, used = el, sel
#                             break
#                     except Exception:
#                         continue
...
#         # JS fallback -- text se click
#         try:
#             found = f.evaluate("""() => {
#                 const want = /credit\\s*\\/?\\s*debit\\s*card/i;
#                 const nodes = [...document.querySelectorAll(
#                     'label, button, [role="radio"], [role="button"], div, span, li')];
#                 const hits = nodes.filter(e => want.test((e.innerText || '').trim())
...
#             }""")
#             if found:
#                 log("      Card JS se chuna: %s" % found)
#                 page.wait_for_timeout(2500)
#                 before_js = card_state(page, f)
#                 page.wait_for_timeout(2000)
#                 if card_chosen(page, f, before_js) or card_fields(f) or gateway_frames(page):
#                     return "Card chun liya (js-text) -- %s" % card_form_ready(page)
#                 used = "js-text"
#         except Exception as e:
#             log("      JS card pick: %s" % str(e)[:60])
#         page.wait_for_timeout(1500)

#     if not loc and used != "js-text":
#         for sel in CARD_SELS:
#             try:
#                 l = page.locator(sel)
#                 if l.count() and l.first.is_visible():
#                     loc, used = l.first, sel + "@page"
#                     f = page
#                     break
#             except Exception:
#                 continue
...
#         try:
#             loc.evaluate("""e => {
#                 const r = e.closest('label')?.querySelector('input[type=radio]')
#                     || e.querySelector('input[type=radio]') || e;
#                 if (r && r.type === 'radio') { r.checked = true; r.click(); }
#                 e.click();
#             }""")
#         except Exception:
#             pass
...
#     if not card_chosen(page, f, before):
#         try:
#             still_upi = f.locator("#src-qrcode-payment-btn").count() > 0
#         except Exception:
#             still_upi = False
#         return "Card daba nahi paye (%s se koshish ki, UPI=%s)" % (
#             how or "kisi tareeke", "abhi bhi" if still_upi else "nahi")
...
#         log("9) Credit/Debit Card chun rahe hain")
#         card_msg = pick_card(pay)
#         log("      %s" % card_msg)
#         if "nahi mila" in card_msg or "nahi paye" in card_msg:
#             log("   FAIL: Card select nahi hua -- ruk rahe hain")
#             try:
#                 pay.screenshot(path=os.path.join(OUT, "order_fail_card_pick.png"))
#                 log("   tasveer: out/order_fail_card_pick.png")
#             except Exception:
#                 pass
#             if a.hold == 0:
#                 try:
#                     input("👉 Dekh lo browser, phir Enter: ")
#                 except Exception:
#                     pay.wait_for_timeout(60000)
#             else:
#                 pay.wait_for_timeout(min(a.hold, 30) * 1000)
#             try:
#                 b.close()
#             except Exception:
#                 pass
...
# -*- coding: utf-8 -*-
"""
simple_order.py -- Simple linear Estuary checkout flow

Flow:
  1. Product page open
  2. Pack select (optional) + Add to cart
  3. Checkout button click
  4. Mobile number fill + OTP (manual / file)
  5. Address form (pincode, name, address, email)
  6. Confirm email button
  7. Credit/Debit Card select
  8. Credit Card type choose
  9. Card details fill + Pay
 10. Corporate ID + Employee ID + Submit
 11. Bank OTP (manual)
 12. Wait / hold

Run:
  python simple_order.py --product black-white-ginger-ale --pack "Pack of 02"
  python simple_order.py --product black-white-ginger-ale --guest --mobile 9999999999
"""
import os
import re
import sys
...
# OTP doctor (otpdoctor.in) -- number khareedne aur OTP paane ke liye. Import
# guard isliye ki module na ho to baaki flow (haath se OTP) chalti rahe.
try:
    import otpd
except Exception:
    otpd = None
...
# ---------- TEST / DEFAULT DATA (CLI se override) ----------
...
def start_proxy_bridge(username):
    local_port = _free_port()
    stop = threading.Event()
    ready = threading.Event()
    ...
    t.start()
    if not ready.wait(timeout=10):
        raise RuntimeError("proxy bridge fail")
    return local_port, stop


def geonode_user(country="in"):
    base = GEONODE_USER_BASE.format(country.lower())
    return "%s-session-%s-lifetime-30" % (base, uuid.uuid4().hex[:10])
...
def click_first(scope, selectors, timeout=5000):
...
def fill_first(scope, selectors, value, label="field"):
    for sel in selectors:
        try:
            loc = scope.locator(sel)
            if loc.count() and loc.first.is_visible(timeout=800):
                ...
                loc.first.fill("")
                loc.first.type(str(value), delay=40)
                log("      %s -> %s" % (label, value))
                return True
        except Exception:
            continue
    log("      %s FAIL (selector nahi mila)" % label)
    return False


def frame_with(page, selector, secs=12):
    """Page ya kisi iframe me selector dhoondo, frame/page return."""
    end = time.time() + secs
    while time.time() < end:
        try:
...
def any_frame_text(page, text_bits):
    """Kisi bhi frame/page me text dikhe to True."""
    bits = [b.lower() for b in text_bits]
    scopes = [page] + list(page.frames)
    for sc in scopes:
        try:
            t = (sc.inner_text("body") or "").lower()
...
def wait_otp_from_file(mobile, secs=300):
    """out/yt_otp_answer.txt me OTP likho — script padh legi."""
    flag = os.path.join(OUT, "otp_wait.flag")
    ans = os.path.join(OUT, "otp_answer.txt")
    try:
    ...
    with open(flag, "w", encoding="utf-8") as fh:
        fh.write("waiting otp for %s\n" % mobile)
    log("      >>> OTP yahan likho: %s  (ya terminal me type) <<<" % ans)
    # Terminal input bhi try (agar interactive)
    otp = ""
    end = time.time() + secs
    while time.time() < end:
        if os.path.exists(ans):
...
def step_add_to_cart(page):
    ...
    try:
        ...
        log("   cart.js read: %s" % e)
    shot(page, "02_cart.png")
    return True


def step_checkout(page, context):
    log("\n[3] Checkout click")
    # cart open if needed
    if not page.locator("#yt-checkout-button, button:has-text('Checkout')").count():
    ...
    new_page = None
    try:
        with context.expect_page(timeout=15000) as ni:
            if not click_first(page, sels):
        ...
        click_first(page, sels)

    pay = new_page or page
    pay.wait_for_timeout(8000)
    shot(pay, "03_after_checkout.png")
    return pay


def step_mobile_otp(pay, mobile, handle=None):
    """handle diya ho to OTP otpdoctor se aata hai (nahi to file/haath se)."""
    log("\n[4] Mobile + OTP")
    scope = frame_with(pay, "input#mobile, input[name='mobile'], input[type='tel']", secs=15)
    if scope is None:
        # maybe already past mobile
        if frame_with(pay, "input#pincode", secs=3) or any_frame_text(pay, ["payment method", "credit/debit"]):
            log("   SKIP: mobile already done / saved")
            return True
    ...
    OTP_BOX_SEL = ('input[aria-label*="Digit"], '
                   'input[autocomplete="one-time-code"], input[maxlength="1"]')
    SEND_SELS = [
        'button:has-text("Continue")', 'button:has-text("Send OTP")',
        'button:has-text("Get OTP")', 'button[type="submit"]',
        'button:has-text("Submit")',
    ]
    click_first(scope, SEND_SELS)
    pay.wait_for_timeout(3000)

    # OTP ke khaane aane tak ruko -- panna dhire load ho ya SMS button ek baar
    # me na chale, isliye 3 baar tak Send dobara daba kar zyada intezaar. Isi
    # se wo dikkat rukti hai jisme "OTP page hi nahi aaya".
    otp_scope = None
    for tryn in range(1, 4):
        otp_scope = frame_with(pay, OTP_BOX_SEL, secs=15)
        if otp_scope is not None:
            break
        # kahin bina OTP ke aage to nahi badh gaya (already verified)?
        if frame_with(pay, "input#pincode", secs=2) or any_frame_text(
                pay, ["payment method", "credit/debit"]):
            log("   SKIP: OTP nahi maanga (already verified)")
            return True
        log("   OTP ke khaane abhi nahi aaye -- Send OTP dobara (%d/3)" % tryn)
        click_first(scope, SEND_SELS)
        pay.wait_for_timeout(4000)
    ...
    if handle is not None and otpd is not None:
        # OTP doctor se: SMS aane tak wo khud poll karta hai.
        log("   OTP otpdoctor se aa raha hai (number %s)" % handle.get("phone"))
        otp = otpd.get_otp(handle, log=log)
        if not otp:
            # ek baar resend keh kar dobara dekho -- pehli SMS kabhi gir jaati hai
            log("   OTP nahi aaya -- resend keh kar dobara dekhte hain")
            try:
                otpd.resend(handle)
            except Exception:
                pass
        ...
        otp = wait_otp_from_file(mobile, secs=300)
        if not otp:
            # fallback: prompt
            try:
    ...
    log("   OTP fill: %s" % otp)
    boxes = otp_scope.locator(
        'input[aria-label*="Digit"], input[autocomplete="one-time-code"], input[maxlength="1"]')
    try:
        n = boxes.count()
    except Exception:
        n = 0
    ...
    # invalid check
    if any_frame_text(pay, ["invalid otp", "incorrect otp", "otp expired", "wrong otp"]):
        log("   FAIL: OTP galat")
        return False
    if handle is not None:
        handle["used"] = True      # number kaam aa gaya -- ise wapas mat karna
    return True


def step_address(pay, a):
    log("\n[5] Address form")
    scope = frame_with(pay, "input#pincode, input[name='pincode']", secs=15)
    if scope is None:
        if any_frame_text(pay, ["payment method", "credit/debit", "confirm your email"]):
    ...
    fill_first(scope, ["input#name", "input[name='firstName']", "input[name='first_name']",
                       "input[placeholder*='first' i]"], a.first, "first")
    fill_first(scope, ["input#lastName", "input[name='lastName']", "input[name='last_name']",
                       "input[placeholder*='last' i]"], a.last, "last")
    ...
    # Home type if present
    try:
        scope.locator("input[name='home']").first.check(timeout=1500)
    except Exception:
        pass
    ...
    if not click_first(scope, [
        ...
        'button:has-text("Save and select")',
        'button:has-text("Save")',
        'button[type="submit"]',
    ]):
        log("   FAIL: Add address / Save button nahi mila")
...
def step_confirm_email(pay, email):
    log("\n[6] Confirm email")
    scope = frame_with(pay, 'input#email, input[name="email"], input[type="email"]', secs=8)

    # Agar confirm text hi nahi to skip
    if not any_frame_text(pay, ["confirm your email", "confirm email"]):
    ...
    for sc in [pay] + list(pay.frames):
        if click_first(sc, btn_sels, timeout=3000):
            clicked = True
    ...
    if not clicked:
        ...
        for sc in [pay] + list(pay.frames):
            try:
                ok = sc.evaluate("""() => {
                    const want = /confirm\\s*email/i;
                    const nodes = [...document.querySelectorAll(
                        'button, a, [role="button"], input[type="submit"]')];
                    const hit = nodes.find(e =>
                        want.test((e.innerText || e.value || '').trim())
                        && e.offsetParent !== null);
                    if (!hit) return false;
                    hit.click();
                    return true;
    ...
    pay.wait_for_timeout(4000)
    if any_frame_text(pay, ["please confirm your email"]) and not any_frame_text(
            pay, ["payment method", "credit/debit"]):
...
def step_pick_card(pay):
    log("\n[7] Credit/Debit Card select")
    # Fastrr / widget frame prefer
    scope = None
    ...
    clicked = click_first(scope, sels, timeout=4000)
    if not clicked:
        for sc in [pay] + list(pay.frames):
            try:
                ok = sc.evaluate("""() => {
                ...
                if ok:
                    clicked = True
                    log("   Card selected (js)")
                    break
            except Exception:
                continue

    if not clicked:
        log("   FAIL: Credit/Debit Card option nahi mila / click nahi hua")
...
def step_card_type(pay, kind="Credit"):
    ...
    while time.time() < end:
        for fr in pay.frames:
            u = (fr.url or "").lower()
            if not any(k in u for k in ("easebuzz", "pay.", "payment", "3ds", "acs", "razorpay")):
                # still try — sometimes same frame
                pass
            if click_first(fr, sels, timeout=2000):
                log("   %s Card clicked" % kind)
                pay.wait_for_timeout(4000)
                shot(pay, "08_card_type.png")
                return True
        if click_first(pay, sels, timeout=1500):
            log("   %s Card clicked (page)" % kind)
            pay.wait_for_timeout(4000)
            return True
        pay.wait_for_timeout(1500)

    log("   WARN: %s Card option nahi mila — card form pehle se open ho sakta hai" % kind)
    return True  # continue — form already open ho sakta hai


def step_fill_card(pay, card, exp, holder, cvv, auto_pay=False):
    log("\n[9] Card details fill")
    # wait form
    fr = None
    end = time.time() + 40
    while time.time() < end:
        for f in pay.frames:
            try:
                if f.locator('input[name="card_number"], input[placeholder="Test Holder"]').count():
                    fr = f
                    break
            except Exception:
                continue
        if fr:
            break
        try:
            if pay.locator('input[name="card_number"]').count():
                fr = pay
                break
        except Exception:
            pass
        pay.wait_for_timeout(1200)
    ...
    fill_first(fr, ['input[placeholder="Test Holder"]',
               ...
               holder, "holder")
    fill_first(fr, ['input[name="card_cvv"]', 'input[placeholder="Test Holder"]'],
               cvv, "cvv")
    ...
    if auto_pay:
        ...
        log("   >>> Pay button MANUALLY dabao, phir yahan Enter <<<")
        try:
            input("👉 Pay ke baad / bank form aaye to Enter: ")
        except Exception:
            pay.wait_for_timeout(60000)
    return True


def step_corp_emp(context, pay, corp_id, emp_id):
...
# ================= MAIN =================
def buy_number(log=log):
    """otpdoctor se ek number. Na mile to None. (retry loop isi ko bulata hai)"""
    if otpd is None:
...
def main():
    ap = argparse.ArgumentParser(description="Simple Estuary order flow")
    ap.add_argument("--product", required=True)
    ap.add_argument("--pack", default="")
    ap.add_argument("--unit", default="")
    ap.add_argument("--session", default="")
    ...
    ap.add_argument("--no-proxy", action="store_true")
    ap.add_argument("--hold", type=int, default=120, help="end wait seconds (0=Enter)")
    ...
    ap.add_argument("--otp-tries", type=int, default=3,
                    help="mobile+OTP kitni baar try karein (har fail par naya number)")
    a = ap.parse_args()

    # Number retry loop me khareeda jaata hai (neeche) -- yahan sirf jagah bana
    # rahe hain. Isse har fail par taaza number milta hai.
    numbuy = None
    ...
    if not a.guest:
        ...
        if path:
            ...
            if os.path.exists(path):
                d = json.load(open(path, encoding="utf-8"))
                storage = d.get("state") or d
                log("session: %s / %s" % (d.get("phone"), d.get("email")))
            else:
                log("session file nahi mili — guest mode")
        else:
            log("koi session nahi — guest mode")
    else:
        log("GUEST mode")
    ...
    if not a.no_proxy:
        try:
            user = geonode_user()
            port, stop_bridge = start_proxy_bridge(user)
            proxy_cfg = {"server": "http://127.0.0.1:%d" % port}
            log("proxy: 127.0.0.1:%d" % port)
        except Exception as e:
            log("proxy fail: %s — bina proxy" % e)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            log("context fail (%s) — proxy hata kar retry" % e)
            ctx_kw.pop("proxy", None)
            ctx = browser.new_context(**{k: v for k, v in ctx_kw.items() if k != "proxy"})
        ctx.add_init_script(
            "Object.defineProperty(navigator,'webdriver',{get:()=>undefined});"
        )
        page = ctx.new_page()
        ...
        try:
            ...
            if not step_add_to_cart(page):
                return 4
            # Checkout + mobile+OTP -- number mar jaye ya OTP ka panna na
            # aaye to naya number le kar DOBARA, har baar taaza checkout se.
            # Taaza checkout isliye ki panna saaf load ho -- adhoore panne par
            # atakne (jaise OTP page na aana) se bacha ja sake.
            pay = None
            mobile_ok = False
            kul_tries = max(1, a.otp_tries)
            for att in range(1, kul_tries + 1):
                log("\n==== mobile+OTP koshish %d/%d ====" % (att, kul_tries))
                if a.buy_number:
                    numbuy = buy_number()
                    if numbuy is None:
                        return 2
                ...
                pay = step_checkout(page, ctx)
                if pay is None:
                    return 5
                if step_mobile_otp(pay, a.mobile, handle=numbuy):
                    mobile_ok = True
                    break
                # fail: number wapas karo, pay tab band karo, phir naya number
                ...
                numbuy = None
                if not a.buy_number:
                    break            # bina number-buy ke dobara ka faayda nahi
                try:
                    if pay is not None and pay != page:
                        pay.close()
                except Exception:
                    pass
                page.wait_for_timeout(3000)
            if not mobile_ok:
                log("   FAIL: %d koshish ke baad bhi mobile+OTP nahi hua"
                    % kul_tries)
                return 6
            if not step_address(pay, a):
                return 7
            if not step_confirm_email(pay, a.email):
                return 8
            if not step_pick_card(pay):
                return 9
            step_card_type(pay, a.card_type)
            if not step_fill_card(pay, a.card, a.exp, a.holder, a.cvv, auto_pay=a.auto_pay):
                return 10
            if not step_corp_emp(ctx, pay, a.corp_id, a.emp_id):
                return 11
            ...
            if a.hold == 0:
                try:
                    input("👉 OTP bhar ke Enter dabao (band karne ke liye): ")
                except Exception:
                    pay.wait_for_timeout(600000)
            else:
                log("   %d sec open rahega..." % a.hold)
                pay.wait_for_timeout(a.hold * 1000)
            ...
            if stop_bridge is not None:
                stop_bridge.set()
            # Number jo kaam nahi aaya use wapas kar do -- warna uska paisa jal
            # jaata hai. Jo OTP tak pahunch gaya (used=True) use chhod dete hain.
            if numbuy is not None and otpd is not None and not numbuy.get("used"):
                try:
                    otpd.cancel(numbuy, log=log)
                except Exception:
...
# def run_once(a, driver=None, own_close=True, acct=None, session_path=None,
#              stop_bridge_in=None):
#     """Ek order lagao. `a` parse_args() se aata hai.

#     Aam istemal: bina kisi extra ke -- session file se browser khulta hai.
#     Combo (single browser): `driver` pehle se khula (account isi me bana),
#     `acct` uski session dict, `session_path` uski file. Tab naya browser nahi
#     khulta, session load nahi hoti, proxy nahi uthti.

#     own_close: sach ho to ant me driver band. (bridge sirf tab band hoti hai
#     jab run_once ne khud uthai ho -- inject ki hui bridge combo ke paas rehti.)

#     Lautata hai: 0 = kaam ho gaya, baaki sab galti ka number.
#     """
#     own_driver_here = driver is None
...
#     if acct is not None:
#         # Combo single-browser: account abhi isi browser me bana hai, session
#         # dict aur file haath me hai -- dobara file se padhne ki zaroorat nahi.
#         d = acct
#         path = session_path or a.session or ""
#     else:
#         path = a.session or newest_session()
#         if not path:
#             log("koi session nahi mili -- pehle make_accounts.py chalao")
...
#     use_proxy = not a.no_proxy
#     stop_bridge = stop_bridge_in
#     bridge_port = None
#     # Proxy sirf tab uthate hain jab browser bhi hum hi khol rahe hain. Inject
#     # kiya hua browser to pehle se apni proxy par chal raha hai.
#     if own_driver_here and use_proxy:
...
#             if not daale:
#                 # Ek bhi cookie na jaane ka matlab hai account logged-out hai.
#                 # Aage badhne par order to lag jaata hai par kisi account se
#                 # nahi juda hota -- pata tab chalta hai jab paisa kat chuka hai.
...
#                 return 2
#         else:
#             # Combo: account isi browser me abhi bana hai -- login ho chuka,
#             # session load karne ki zaroorat nahi.
#             log("browser: pehle se khula (account isi me bana) -- "
#                 "seedha order par")
...
#         if driver is not None and own_close and not a.no_save_session and path:
#             log("\n   session band karne se pehle taaza cookies utha rahe hain...")
#             try:
#                 save_session(driver, d, path)
#             except Exception as e:
#                 log("   session save fail: %s" % str(e)[:100])
#         if driver is not None and own_close:
#             try:
#                 driver.quit()
#             except Exception:
#                 pass
#         # Bridge sirf tabhi band karo jab humne khud uthai ho. Inject ki hui
#         # bridge combo ki hai -- aage ke chakkaron ke liye chalti rehni chahiye.
#         if stop_bridge is not None and own_driver_here:
#             stop_bridge.set()
#     return 0
