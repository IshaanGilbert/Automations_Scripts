...
"""EstuaryComboRunner -- account banao aur usi par turant order lagao.

Ye chauthi exe hai aur akeli aisi hai jo dono kaam ek saath karti hai. Baaki
teen apni jagah waisi ki waisi rehti hain: do sirf account banati hain
(mobile+OTP wali aur email wali), teesri sirf order lagati hai un accounts par
jo pehle se bane pade hain.

Farak sirf beech ke kadam ka hai. Alag-alag chalane par ye dekhna padta tha ki
kaun sa naya account bacha hai aur kaun sa use ho chuka -- yahan wo sawaal
uthta hi nahi: jo account abhi bana, order usi par lagta hai. Isliye is exe ke
saath purani `sessions\` copy karne ki bhi zaroorat nahi -- accounts yahin
bante hain.

Ek chakkar = ek email row (Account_emails) + ek card (payment_details).
...
BANANA
    pyinstaller --noconfirm EstuaryComboRunner.spec
    # nateeja: dist\EstuaryComboRunner\EstuaryComboRunner.exe
"""
import os
import json
...
exe = EXE(
    ...
    name="EstuaryComboRunner",
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=False,
    # GUI hai, isliye console nahi. Har line window ke log me aur
    # gui_log_combo.txt me jaati hai -- console na hone par bhi kuch chhupta nahi.
    console=False,
