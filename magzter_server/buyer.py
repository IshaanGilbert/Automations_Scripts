...
import sheet_db as db   # Google Sheet backed task store (Postgres db.py se replace)
...
# sheet_db ko instance number batao (creds1..6 round-robin -> Google rate-limit spread)
os.environ["BUYER_INSTANCE"] = str(INSTANCE_ID)

print(f"🤖 Instance {INSTANCE_ID}/{TOTAL_INSTANCES}")

# ================= CONFIG =================
# EXE (PyInstaller) me chalte waqt kaam-kaaj exe ke folder se ho -> config.json,
# logs, screenshots sab exe ke bagal me banein/padhe jayein (bundled temp se nahi).
...
# CARD-MATCH: OTP API 'card_last_four' se match kare ya nahi. Default True (safe: dusre
# card ka OTP na uthe). Par otp_lock pehle se per-employee OTP window serialize karta hai,
# isliye ek waqt me EK hi card ka OTP-flow chalta hai -> matching redundant ho jaati hai
# aur galat/stale tag ki wajah se sahi OTP bhi reject ho sakta hai. Agar trackopia ka
# card tag galat/stale aata hai to config me "OTP_REQUIRE_CARD_MATCH": false kar do —
# tab bot latest OTP le lega (otp_lock ki wajah se wahi is card ka hi hoga).
OTP_REQUIRE_CARD_MATCH = str(config.get("OTP_REQUIRE_CARD_MATCH", True)).lower() not in ("0","false","no","off")
...
# FAST-FAIL on a real hCaptcha challenge (user rule: "captcha aaye to wait mat karo, turant
# retry"). The instant "I am human" checkbox is still tried once (it often passes free); but
# if that does NOT pass — an actual image challenge — we re-queue the task immediately
# instead of stalling ~2 min waiting for the corp form. A fresh retry (new proxy/fingerprint)
# usually lands captcha-free. Turn off with CAPTCHA_FAST_FAIL=0 to wait/solve as before.
CAPTCHA_FAST_FAIL = str(config.get("CAPTCHA_FAST_FAIL", os.environ.get("CAPTCHA_FAST_FAIL", "1"))).lower() not in ("0","false","no","off")
...
RETRYABLE_ERRORS = [
    ...
    "Captcha appeared",
    # late "not eligible" (surfaced only at the corp step): we've saved a mutated email and
    # want the whole flow re-run with it (the retry reuses the SAME reserved card).
    "Eligibility mutation retry",
]
MAX_TASK_RETRIES = 3
            ...
            if sc == 200:
                try:
                    ...
                    if otp:
                        ...
                        # OTP_REQUIRE_CARD_MATCH=false -> match ignore karo (otp_lock already
                        # ek employee pe ek hi card ka OTP-window chalata hai, to latest OTP
                        # isi card ka hai; galat/stale tag se sahi OTP reject na ho).
                        if OTP_REQUIRE_CARD_MATCH and want4 and c4 and c4 != want4:
                            if i % 5 == 0:
                                print(f"   ⏳ OTP card mismatch (API ****{c4} ≠ task ****{want4}) — waiting…", flush=True)
                            time.sleep(delay); continue
                        if want4 and c4 and c4 != want4:
                            print(f"   ⚠️ OTP card tag mismatch (API ****{c4} ≠ ****{want4}) — match OFF, using anyway", flush=True)
...
ELIGIBILITY_MSGS = [
    ...
    # existing-GOLD-user offer block (surfaces on Billing Details, before Stripe loads).
    # Without these it was misread as "Stripe payment form not loaded" and email mutation
    # never fired — a mutated/fresh email IS eligible, so this must trigger the retry.
...
""")
        ...
        _orig_email = email
        _tried = {email}
        _renav = False          # retry re-navigates for a fresh reCAPTCHA/IP (login/verify fix)
        _renav_count = 0
        while True:
            try:
                if _renav:
                    # A previous attempt got stuck on /login/verify (missing/weak reCAPTCHA
                    # token). Re-navigate to the offer + Claim so the email step runs fresh
                    # (fresh proxy IP + fresh reCAPTCHA => valid token => reaches billing).
                    _renav = False
                    print('   ↻ re-navigating for fresh reCAPTCHA/IP…', flush=True)
                    if not safe_goto(page, TARGET_URL): raise Exception("Navigation failed")
                    time.sleep(random.uniform(3, 5)); dismiss_popups(page)
                    if not click_with_retry(page, ['button:has-text("Claim Now")','button.yellow_btn','a.subscribe_a button']):
                        raise Exception("Claim button not found")
                    time.sleep(random.uniform(3, 5)); dismiss_popups(page)
                ...
                # ── LOGIN/VERIFY INTERSTITIAL = force-submit dropped the offer URL params ──
                # The email step's force-submit (no valid reCAPTCHA token) lands on
                # /login/verify (or /login/email, /signin) and the offer URL params
                # (planType/type/fms/from) get DROPPED. Clicking Continue/Skip or re-typing the
                # email on this page does NOT re-run eligibility — it only WASTES time. So do
                # NOT linger here: immediately RE-NAVIGATE to the offer (restores the params +
                # fresh reCAPTCHA/IP) and re-enter the SAME email — that reaches billing (proven
                # by testing). No mutation, no Continue-click retries, no waiting.
                try:
                    ...
                    if _is_login_url and 'checkout/summary' not in page.url:
                        if _renav_count < MAX_EMAIL_MUTATIONS:
                            _renav_count += 1
                            print(f'   🔁 login interstitial → re-hitting target URL #{_renav_count} (same email, no time-waste)', flush=True)
                            _renav = True
                            continue
        ...
        while time.time() < _stripe_hard_deadline:
            ...
            if _card_loaded: break
            # USER RULE: a captcha on the payment page means Stripe will NEVER open its card
            # form here — do NOT sit through the whole ~210s wait / reloads. Fast-fail to
            # retry immediately (a fresh attempt from another IP usually lands captcha-free).
            if CAPTCHA_FAST_FAIL and _hcaptcha_present(page):
        ...
        if not _card_loaded:
            ...
            # The card form never loaded — but "Stripe not loaded" is usually a SYMPTOM, not
            # the cause. Attribute the REAL reason so the dashboard error reads truthfully:
            #   1) a CAPTCHA on the page (Stripe never opens behind it) -> report Captcha
            #   2) the site is DOWN / rate-limiting (503) -> report Site unavailable
            #   3) an ELIGIBILITY block ("existing GOLD user") -> report Not eligible (mutate)
            # Each is retryable and lands the operator on the true cause, not a generic error.
            if _hcaptcha_present(page):
                raise Exception("Captcha appeared — fast-fail to retry")
            _down = page_has_error(page, SITE_DOWN_MSGS)
            if _down:
                raise Exception(f"Site unavailable ({_down}) - retry")
            _elig = page_has_error(page, ELIGIBILITY_MSGS)
            if _elig:
                raise Exception(f"Not eligible: {_elig}")
        ...
        # A late "not eligible" (account already exists for this email) can surface only when
        # the corporate form should appear — the corp form then never loads and we USED to
        # mislabel it "Corporate form not found" and retry with the SAME email forever. Detect
        # it here and, like the email/billing step, mutate to a fresh email + persist
        # updated_email + RE-QUEUE. The retry reuses the SAME reserved card (no second card is
        # charged); the next run redoes the flow from the top with the mutated email.
        def _eligibility_requeue(elig_err):
            if not EMAIL_MUTATION or len(_tried) > MAX_EMAIL_MUTATIONS:
                raise Exception(f"Not eligible: {elig_err}")            # terminal ineligible
            _nxt = db.mutate_email(_orig_email, _tried)
            if not _nxt:
                raise Exception(f"Not eligible: mutations exhausted ({str(elig_err)[:40]})")
            _tried.add(_nxt)
            try: db.set_updated_email(task_id, _nxt, len(_tried) - 1)
            except Exception: pass
            print(f"   🔁 Not eligible at corp step — saved mutated email {_nxt}; re-queuing "
                  f"to redo the flow with it.", flush=True)
            raise Exception("Eligibility mutation retry")              # retryable (see list)
        ...
        for corp_try in range(1, 4):                      # 3 attempts (2 reloads)
            ...
            _cap = "none"
            try:
                if time.time() < _cap_deadline: _cap = solve_hcaptcha(page)
            except Exception:
                # A TRANSIENT solver error (frame detached / context destroyed during the
                # post-nav churn) is NOT the same as a real unsolvable challenge — treating it
                # as one fast-failed perfectly good buys. Fall through as "none" and keep
                # polling; only an EXPLICIT False from solve_hcaptcha means a real block.
                _cap = "none"
            # FAST-FAIL: the checkbox did NOT pass => a real captcha challenge is blocking the
            # corp form. Don't burn ~2 min waiting (or pay the solver) — re-queue now.
            if CAPTCHA_FAST_FAIL and _cap is False:
                raise Exception("Captcha appeared — fast-fail to retry")
            for n in range(40):                           # 40s/try × 3 = ~2 min (was 6 min)
                ...
                if n % 5 == 0:
                    ...
                    elig_err = page_has_error(page, ELIGIBILITY_MSGS)
                    if elig_err: _eligibility_requeue(elig_err)   # mutate email + re-queue
            ...
            if corp_try < 3:
                ...
                dismiss_popups(page)
        if not corp_ctx:
            # Before blaming the corp form: was it actually a "not eligible" page? If so,
            # mutate the email + re-queue instead of the misleading "Corporate form not found".
            elig_err = page_has_error(page, ELIGIBILITY_MSGS)
            if elig_err: _eligibility_requeue(elig_err)
            try:
        ...
        with db.otp_lock(emp_id):
            ...
            # OTP screen detection
            print("   ✅ Corporate submitted, waiting for OTP screen...")
